...
@needcheck()
class TaskHandler(AccessHandler):

    def get(self):
        #获得用户课程习题
        course_task = self.get_json_argument('coursetaskid','')
    ...
    def delete(self):
        #删除习题答案
        userid = self.uid
        taskerid = self.get_json_argument('taskerid')
...
@needcheck()
class TaskUnfinishedHndler(AccessHandler):
    """获得用户作业完成统计信息"""
    def get(self):
        #标记习题是否完成
